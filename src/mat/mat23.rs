...
#[cfg(feature = "f32")]
use crate::{
    Mat2x4, Mat2x8, Mat3x4, Mat3x8, Mat32x4, Mat32x8, SymmetricMat3x4, SymmetricMat3x8, Vec2x4,
    Vec2x8, Vec3x4, Vec3x8,
};

macro_rules! wide_mat23s {
    ($($n:ident => $nonwiden:ident, $m32t:ident, $symmetricm3t:ident, $m2t:ident, $m3t:ident, $v2t:ident, $v3t:ident, $t:ident),+) => {
        $(
        /// A wide 2x3 column-major matrix.
        #[derive(Clone, Copy)]
        ...
        impl $n {
            ...
            /// Creates a new 2x3 matrix with all lanes set to `m`.
            #[inline]
            #[must_use]
            pub fn splat(m: $nonwiden) -> Self {
                Self {
                    x_axis: $v2t::splat(m.x_axis),
                    y_axis: $v2t::splat(m.y_axis),
                    z_axis: $v2t::splat(m.z_axis),
                }
            }
...
#[cfg(feature = "f32")]
wide_mat23s!(
    Mat23x4 => Mat23, Mat32x4, SymmetricMat3x4, Mat2x4, Mat3x4, Vec2x4, Vec3x4, f32x4,
    Mat23x8 => Mat23, Mat32x8, SymmetricMat3x8, Mat2x8, Mat3x8, Vec2x8, Vec3x8, f32x8
);
